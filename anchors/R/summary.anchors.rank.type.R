...
summary.anchors.rank.type <- function(A) {
  ...
  scalar.matrix <- as.data.frame(list(N=as.double(scalar.freq),
                                      Prop=as.double(scalar.freq/n.scalar),
                                      from=nn,to=nn))
  ...
  unif.matrix <- NULL
  if (!is.null(A$weight)) {
    unif.matrix <- as.data.frame(list(N=as.double( apply(A$weight,2,sum) ),
                                      Prop = as.double( apply(A$weight,2,mean) ),
                                      from= 1:A$max,
                                      to  = 1:A$max))
  ...
  interval.matrix <- as.data.frame(list(                           
                           N=as.double(scalar.freq),
                           Prop=as.double(scalar.freq/N),
                           from=nn,to=nn ))
  nn <- names(scalar.freq)
  nn2<- ifelse( nchar(nn)<2, paste(" ",nn,sep=""), nn)
